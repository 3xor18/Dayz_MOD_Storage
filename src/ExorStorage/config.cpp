...
class CfgVehicles
{
	...
	// ------------------------------------------------------------------
	//  MALETIN DEL EVENTO - objetivo del evento de escolta (ExorMaletin.c).
	//  Es el maletin tecnico vanilla. Se hereda en vez de usar el original para
	//  poder colgarle el humo y el hook de movimiento SIN tocar el item del juego
	//  (un maletin que aparece en el loot normal tiene que seguir siendo normal).
	// ------------------------------------------------------------------
	class ScientificBriefcase;	// externa (DZ_Gear_Containers)
	class Exor_MaletinEvento: ScientificBriefcase
	{
		scope = 2;
		displayName = "Maletin del evento";
		descriptionShort = "Hay que llevarlo hasta el punto de entrega. No se puede soltar y todo el server te ve en el mapa.";
		// pesa poco a proposito: el evento ya es bastante castigo sin encima frenarlo
		weight = 2500;
	};

	// ------------------------------------------------------------------
	//  COFRE DE LOOT - cofres camuflados fijos en el mapa (ExorCofreLoot.c).
	//  Es el baul marino vanilla (SeaChest) con la textura camuflada del mod: el
	//  SeaChest declara hiddenSelections {"camoGround"}, asi que alcanza con pisarle
	//  la textura; el relieve y el brillo salen del material vanilla, gratis.
	//  - weight enorme + rotationFlags 2: se queda donde lo ponen.
	//  - canBeDigged = 0: no se entierra (seria una via para "moverlo" igual).
	//  - Vida altisima A PROPOSITO: la "vida" real del cofre es el contador de
	//    golpes/tiros del modulo. Si los hitpoints fueran normales, 20 balazos lo
	//    dejarian ARRUINADO -y el cargo inservible- antes de llegar a abrirse.
	//  - openable = 0: el cargo no tiene animacion de tapa; lo que decide si se ve
	//    es CanDisplayCargo() del script (cerrado = no se ve nada adentro).
	// ------------------------------------------------------------------
	class Exor_CofreLoot: SeaChest
	{
		scope = 2;
		displayName = "Cofre";
		descriptionShort = "Cofre de suministros. No se puede levantar: hay que reventarlo a golpes o a tiros.";
		hiddenSelectionsTextures[] = {"ExorStorage\data\cofres\exor_cofre_camo_co.paa"};
		weight = 250000;
		canBeDigged = 0;
		carveNavmesh = 1;
		rotationFlags = 2;
		class Cargo
		{
			itemsCargoSize[] = {10, 20};	// 200 slots: entra un set completo con attachments
			openable = 0;
			allowOwnedCargoManipulation = 1;
		};
		class DamageSystem
	...
	// ------------------------------------------------------------------
	//  BALIZA DEL PUNTO DE ENTREGA (evento del maletin). Hereda de la luz del
	//  evento de cofres, que ya es una bengala fija y no agarrable; lo unico
	//  que suma es el humo de color. Se borra cuando llega el maletin.
	// ------------------------------------------------------------------
	class Exor_HumoEvento: Exor_CofreLight
	{
		scope = 2;
		displayName = "Punto de entrega";
		descriptionShort = "Baliza del evento del maletin. Fija, no se puede tomar.";
	};
